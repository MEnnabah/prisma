...
`
  ...
  private renderRelations(projection: Projection): string {
    ...
    if (projection === Projection.include) {
      ...
      if (nonCompositeRelations.length === 0) {
        return ''
      } else {
        relations = nonCompositeRelations
      }
    }
